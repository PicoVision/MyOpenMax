...
# Threshold Loop
threshold = 0.9
for(threshold in seq(0,0.9,by=0.01)){
  ...
  Data_select_closeset <- within(Data_select_closeset, ST_Re <- (Data_select_closeset$SoftMaxPredict>=1000)*1000)  
  
  #remap openset
  Data_select_openset <- within(Data_select_openset, GT_Re <- (Data_select_openset$GT_Index>=1000)*1000)  
  Data_select_openset <- within(Data_select_openset, ST_Re <- (Data_select_openset$SoftMaxPredict>=1000)*1000)  
  ...
  ACCCloseset = sTN/50000
  ACCOpenset = sTP/15000
  ...
  cat("ACC = ",ACC,"\n")
  Data_ACC = rbind(Data_ACC,data.frame("mAP"=ACC,"AccClose"=ACCCloseset,"ACCOpenset"=ACCOpenset,  "Threshold"=threshold))
}
write.csv(Data_ACC,"result_SoftACC.csv")
#read Data section
Data_ACC = read.csv("result_SoftACC.csv")

p = ggplot(Data_ACC,aes(x=Threshold,y=ACCOpenset))+geom_point(color='blue')
p  
