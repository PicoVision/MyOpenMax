...
for (Stepdataamount in seq(30000,75000,by=15000)){
  ...
  for(threshold in seq(0,0.9,by=0.05)){
    ...
    Data_select_Hacked[which(Data_select_Hacked$Prob_Of_Openmax<threshold),"OpenMaxPredict"] = 1000
    ...
    #closetset
    Data_select_closeset[Data_select_closeset$OpenMaxPredict == Data_select_closeset$GT_Index,c("TP")]=1
    Data_select_closeset[Data_select_closeset$OpenMaxPredict != Data_select_closeset$GT_Index,c("FP")]=1
    
    
    #remap openset
    Data_select_openset[Data_select_openset$OpenMaxPredict != Data_select_openset$GT_Index,c("FN")]=1
    Data_select_Hacked[Data_select_Hacked$OpenMaxPredict != Data_select_Hacked$GT_Index,c("FN")]=1
...
write.csv(Data_result,"result_Fscore.csv")
#read Data section
Data_OFC = read.csv("result_Fscore.csv")
Data_OFC["class"] = "openmax"
Data_SFC = read.csv("result_sFscore.csv")

Data_SFC["class"] = "softmax"
Data_FC = rbind(Data_OFC,Data_SFC)
Data_FC$interact <- interaction(Data_FC$class, Data_FC$OpensetElement)



p = ggplot(Data_result,aes(x=Threshold,y=F1,group=OpensetElement,color=factor(OpensetElement)))+geom_point()
p  


p = ggplot(Data_FC,aes(x=Threshold,y=F1,group=interact,color=class))+geom_point()+ geom_line()
p  

ggplot(Data_FC, aes(x=Threshold,y=F1, color = factor(OpensetElement),linetype=factor(class), 
                shape = factor(class)))+geom_point()+geom_line()
